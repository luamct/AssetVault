...
TextureCacheEntry TextureManager::get_asset_texture(const Asset& asset) {
  ...
  LOG_DEBUG("[TextureManager] Cache miss for '{}', creating new entry", relative_path);
  ...
  if (asset.type == AssetType::_3D) {
    ...
    if (std::filesystem::exists(thumbnail_path)) {
      LOG_DEBUG("[TextureManager] 3D model '{}': thumbnail exists, loading", relative_path);
      ...
      if (texture_id != 0) {
        ...
        entry.loaded = true;
        LOG_DEBUG("[TextureManager] 3D model '{}': thumbnail loaded, texture_id: {}", 
                  relative_path, texture_id);
        return entry;
      } else {
        LOG_DEBUG("[TextureManager] 3D model '{}': failed to load thumbnail", relative_path);
      }
    } else {
      LOG_DEBUG("[TextureManager] 3D model '{}': no thumbnail exists", relative_path);
    }
    ...
    LOG_DEBUG("[TextureManager] 3D model '{}': using default icon, texture_id: {} (thumbnail pending)", 
  ...
  if (asset.type != AssetType::_2D) {
    ...
    entry.width = Config::THUMBNAIL_SIZE;
    entry.height = Config::THUMBNAIL_SIZE;
    LOG_DEBUG("[TextureManager] Non-2D asset '{}' ({}): using type icon, texture_id: {}", 
  ...
  if (!std::filesystem::exists(std::filesystem::u8path(asset.path))) {
    auto icon_it = type_icons_.find(asset.type);
    entry.default_texture_id = (icon_it != type_icons_.end()) ? icon_it->second : default_texture_;  // Use default_texture_id for shared icon
    LOG_DEBUG("[TextureManager] 2D asset '{}': file doesn't exist, using default icon, texture_id: {}", 
  ...
  if (asset.extension == ".svg") {
    LOG_DEBUG("[TextureManager] 2D asset '{}': loading SVG", relative_path);
    texture_id = load_svg_texture(asset.path.c_str(), Config::SVG_THUMBNAIL_SIZE, Config::SVG_THUMBNAIL_SIZE, &width, &height);
    if (texture_id != 0) {
      LOG_DEBUG("[TextureManager] 2D asset '{}': SVG loaded, texture_id: {}, size: {}x{}", 
                relative_path, texture_id, width, height);
    }
  }
  else {
    // Load regular texture using stb_image
    LOG_DEBUG("[TextureManager] 2D asset '{}': loading texture", relative_path);
    texture_id = load_texture(asset.path.c_str(), &width, &height);
    if (texture_id != 0) {
      LOG_DEBUG("[TextureManager] 2D asset '{}': texture loaded, texture_id: {}, size: {}x{}", 
                relative_path, texture_id, width, height);
    }
  }
  ...
  if (texture_id == 0) {
    ...
    entry.width = Config::THUMBNAIL_SIZE;
    entry.height = Config::THUMBNAIL_SIZE;
    LOG_DEBUG("[TextureManager] 2D asset '{}': failed to load, using default icon, texture_id: {}", 
              relative_path, entry.default_texture_id);
    return entry;
  }
...
void TextureManager::queue_texture_invalidation(const std::string& file_path) {
  ...
  LOG_DEBUG("[TEXTURE] Queued invalidation for: {}", file_path);
}
  ...
  while (!invalidation_queue_.empty()) {
    ...
    if (cache_it != texture_cache_.end()) {
      ...
      if (cache_it->second.texture_id != 0 && cache_it->second.default_texture_id == 0) {
        LOG_DEBUG("[TEXTURE] Processing invalidation for: {} (deleting owned texture_id: {})", 
                  file_path, cache_it->second.texture_id);
        glDeleteTextures(1, &cache_it->second.texture_id);
      } else if (cache_it->second.default_texture_id > 0) {
        LOG_DEBUG("[TEXTURE] Processing invalidation for: {} (keeping shared default_texture_id: {})", 
