...
bool Import3DFromFile(const std::string& pFile) {
  ...
  if (!fin.fail()) {
    ...
    MessageBox(
      nullptr, UTFConverter("Couldn't open file: " + pFile).c_wstr(), TEXT("ERROR"), MB_OK | MB_ICONEXCLAMATION
    );
...
int LoadGLTextures(const aiScene* scene) {
  ...
  for (size_t i = 0; i < numTextures; i++) {
    ...
    if (nullptr != data) {
      ...
      /* Error occurred */
      MessageBox(
        nullptr, UTFConverter("Couldn't load Image: " + fileloc).c_wstr(), TEXT("ERROR"), MB_OK | MB_ICONEXCLAMATION
      );
    }
  }
...
void KillGLWindow() // Properly Kill The Window
{
  ...
  if (hRC) // Do We Have A Rendering Context?
  {
    ...
    {
      MessageBox(
        nullptr, TEXT("Release Rendering Context Failed."), TEXT("SHUTDOWN ERROR"), MB_OK | MB_ICONINFORMATION
      );
    }
    hRC = nullptr;
  }
...
BOOL CreateGLWindow(const char* title, int width, int height, int bits, bool fullscreenflag) {
  ...
  if (fullscreen) // attempt fullscreen mode
  {
    ...
    if (ChangeDisplaySettings(&dmScreenSettings, CDS_FULLSCREEN) != DISP_CHANGE_SUCCESSFUL) {
      ...
      if (MessageBox(
            ...
            TEXT("NeHe GL"), MB_YESNO | MB_ICONEXCLAMATION
          ) == IDYES) {
  ...
  if (nullptr == (g_hWnd = CreateWindowEx(
...
LRESULT CALLBACK WndProc(
  HWND hWnd,     // Handles for this Window
...
int WINAPI WinMain(
  HINSTANCE /*hInstance*/,     // The instance
...
)                              // Window Show State
{
  ...
  if (MessageBox(
        nullptr, TEXT("Would You Like To Run In Fullscreen Mode?"), TEXT("Start Fullscreen?"),
